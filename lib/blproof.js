...
function extract_partial_tree (complete_tree, user) {
    var partial_tree = null;

    if (complete_tree instanceof Tree) {
        partial_tree = complete_tree.clone();
    } else {
        partial_tree = new Tree()
        partial_tree.arr = JSON.parse(JSON.stringify(complete_tree.arr));
    }
